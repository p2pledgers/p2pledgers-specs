...
The contracts then get normalized, hashed, and enveloped in JSON before signing and sharing (see Envelopes). This ensures they can be enriched with metadata like references (see Folio IDs), shared incrementally (see Gossip), and signed using cryptocurrency hardware if desired (see Signing Hardware).
